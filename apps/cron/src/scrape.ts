...
export async function probeArchivePage(
  ...
  const url = page <= 1 ? `${sourceBase}/archives` : `${sourceBase}/archives?page=${page}`;
  try {
...
export async function discoverFromSitemap(
  ...
  const candidates = [`${sourceBase}/sitemap.xml`, `${sourceBase}/sitemap_index.xml`];
  ...
  for (const candidate of candidates) {
    const urls = await collectSitemapUrls(candidate, userAgent, new Set<string>());
    if (urls.length > 0) {
